...
int main() {
  ...
  double max_s = 6945.554;
    ...
    //variable denoting lane
  int lane = 1;
  //Reference velocity below which we want car to travel
          ...
          // j[1] is the data JSON object
            ...
            if(too_close)
            {
                ...
                if (lane == 1 && lane_change==false)
                {
                    ...
                    std::cout<<"Trying lane 1 to 0. trying left turn.."<<endl;
                    for (int i =0; i < sensor_fusion.size(); i++)
                    {
                        float d =  sensor_fusion[i][6];
                        ...
                        if(d < 4 && d >0)
                        {
                            ...
                            double left_vy = sensor_fusion[i][4];
                            double left_check_speed = sqrt(left_vx*left_vx + left_vy*left_vy);
                            double left_check_car_s = sensor_fusion[i][5];

                            left_check_car_s += ((double) prev_path_size * .02 * left_check_speed);
                            ...
                            if((sensor_fusion[i][5] >(current_car_s-5) && sensor_fusion[i][5] <(car_s+15))||((abs(car_s - left_check_car_s) < 20)))
                            {
                                lane_change = false;
                                std::cout<<"break"<<endl;
                                break;
                    ...
                    //if left turn attempt fails check if we can take right lane
                    if(lane_change==false)
                    {
                        std::cout<<"failed-lane1-left_lane_change attempt. trying right turn.."<<endl;
                        for (int i =0; i < sensor_fusion.size(); i++)
                        {
                            float d =  sensor_fusion[i][6];
                            ...
                            if(d < 12 && d >8)
                            {
                                ...
                                double right_vy = sensor_fusion[i][4];
                                double right_check_speed = sqrt(right_vx*right_vx + right_vy*right_vy);
                                ...
                                //Check if there is any vehicle in the path we are planning to take while changing left lane. Come out of loop if there is any vehicle
                                if((sensor_fusion[i][5] >(current_car_s-5) && sensor_fusion[i][5] <(car_s+15))||((abs(car_s - right_check_car_s) < 20)))
                                {
                                    lane_change = false;
                                    std::cout<<"break"<<endl;
                                    break;
			...
			// List of wildly sparsed (x,y) waypoints evenly spaced at 30m
            //Later this points can be interpolated with spline and fill it with more points to control speed

			vector<double> ptsx;
			vector<double> ptsy;

            // reference x, y and car yaw
			double ref_x = car_x;
			double ref_y = car_y;
			double ref_yaw = deg2rad(car_yaw);

            // if prev_path_size is almost empty, use the car as starting reference
            if(prev_path_size < 2)
            {
            ...
            // define the actula waypoints vector
            ...
            // calculate how to breakup spline points so that we travel with our desired reference velocity
            ...
            for (int i = 1; i < 50 - prev_path_size; i++)
            {
                ...
                // transform into global co-ordinate system
